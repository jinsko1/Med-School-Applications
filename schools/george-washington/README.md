# George Washington University School of Medicine & Health Sciences

- Active list location: Washington, DC
- Active list COA/year: 108572
- Active list GPA median: 3.9
- Active list MCAT median: 515
- Latest prompt cycle used here: 2025-2026 from Med School Insiders
- Latest verified secondary deadline: December 11, 2025
- Official admissions page: https://smhs.gwu.edu/academics/md-program/admissions
- Prompt source: https://medschoolinsiders.com/medical-school-secondary-prompts-database/george-washington-secondary-essay-prompts/

## Required Core Items
- AMCAS primary application
- Official transcripts sent to AMCAS
- Letters of evaluation assigned through AMCAS
- MCAT score on file
- School-specific secondary application when invited

## School-Specific Notes
- Strong service, advocacy, public health, and policy fit, but the live MSAR stats make this a reach rather than a true odds-improver.
- Secondary prompt count captured in this repo: 7

## Essay Packet
- `schools/george-washington/essay-packet.typ`
- `schools/george-washington/essays/prompt-01-regional-medical-campus-interest.md`
- `schools/george-washington/essays/prompt-01.draft.md`
- `schools/george-washington/essays/prompt-02-application-through-matriculation-activities.md`
- `schools/george-washington/essays/prompt-02.draft.md`
- `schools/george-washington/essays/prompt-03-clinical-public-health-interests.md`
- `schools/george-washington/essays/prompt-03.draft.md`
- `schools/george-washington/essays/prompt-04-uniqueness-challenges-and-learning.md`
- `schools/george-washington/essays/prompt-04.draft.md`
- `schools/george-washington/essays/prompt-05-specific-interest-in-gw.md`
- `schools/george-washington/essays/prompt-05.draft.md`
- `schools/george-washington/essays/prompt-06-prior-admissions-office-interaction.md`
- `schools/george-washington/essays/prompt-06.draft.md`
- `schools/george-washington/essays/prompt-07-joint-md-mph-interest.md`
- `schools/george-washington/essays/prompt-07.draft.md`

## School Metadata
- Why-school fact: GW is located a few blocks from the White House and pairs public-health/policy proximity with a 17,000+ square-foot Clinical Learning and Simulation Skills Center.
- Estimated admit chance: ~2% (reach)
- Estimate basis: Heuristic, not a true admissions probability. Assumes CA resident, GPA 3.75, MCAT 517, and strong research/service/clinical fit. Uses cached 2027 MSAR-style total applicant/interview/matriculant counts (17194 applied, 746 interviewed, 130 matriculated when available), school medians GPA 3.9 / MCAT 515, private or geography-neutral pool, and research alignment, service alignment, public-health alignment.
