# Albany Medical College

- Active list location: Albany, NY
- Active list COA/year: 93727
- Active list GPA median: 3.87
- Active list MCAT median: 513
- Latest prompt cycle used here: 2025-2026 from Admit.org / Med School Insiders
- Latest verified secondary deadline: December 1, 2025
- Official admissions page: https://www.amc.edu/academic/undergraduate-medical-education/admissions/
- Prompt source: https://med.admit.org/secondary-essays/albany-medical-college

## Required Core Items
- AMCAS primary application
- Official transcripts sent to AMCAS
- Letters of evaluation assigned through AMCAS
- MCAT score on file
- School-specific secondary application when invited

## School-Specific Notes
- Large prompt set with strong overlap in challenge, community, and social-determinants essays.
- Secondary prompt count captured in this repo: 8

## Essay Packet
- `schools/albany/essay-packet.typ`
- `schools/albany/essays/prompt-01-describe-yourself.md`
- `schools/albany/essays/prompt-01.draft.md`
- `schools/albany/essays/prompt-02-academic-inconsistencies.md`
- `schools/albany/essays/prompt-02.draft.md`
- `schools/albany/essays/prompt-03-interrupted-attendance-or-gaps.md`
- `schools/albany/essays/prompt-03.draft.md`
- `schools/albany/essays/prompt-04-significant-challenge.md`
- `schools/albany/essays/prompt-04.draft.md`
- `schools/albany/essays/prompt-05-structural-and-social-determinants-of-health.md`
- `schools/albany/essays/prompt-05.draft.md`
- `schools/albany/essays/prompt-06-community-identity.md`
- `schools/albany/essays/prompt-06.draft.md`
- `schools/albany/essays/prompt-07-anything-else.md`
- `schools/albany/essays/prompt-07.draft.md`
- `schools/albany/essays/prompt-08-most-meaningful-activity-impact.md`
- `schools/albany/essays/prompt-08.draft.md`

## School Metadata
- Why-school fact: Albany Medical College is embedded in the Albany Med Health System, a large regional system with four hospitals, 1,520 beds, more than 800 physicians, and 125 outpatient locations.
- Heuristic list estimate: ~3%
