# Chicago Medical School at Rosalind Franklin University of Medicine and Science

- Active list location: North Chicago, IL
- Active list COA/year: 104743
- Active list GPA median: 3.79
- Active list MCAT median: 510
- Latest prompt cycle used here: 2025-2026 from Med School Insiders
- Latest verified secondary deadline: December 1, 2025
- Official admissions page: https://www.rosalindfranklin.edu/admission-aid/
- Prompt source: https://medschoolinsiders.com/medical-school-secondary-prompts-database/chicago-med-school-rosalind-franklin-secondary-essay-prompts/

## Required Core Items
- AMCAS primary application
- Official transcripts sent to AMCAS
- Letters of evaluation assigned through AMCAS
- MCAT score on file
- School-specific secondary application when invited

## School-Specific Notes
- Very concise 150-word essays; useful to draft as tight vignettes instead of traditional long-form secondaries.
- Secondary prompt count captured in this repo: 6

## Essay Packet
- `schools/rosalind-franklin/essay-packet.typ`
- `schools/rosalind-franklin/essays/prompt-01-why-medicine.md`
- `schools/rosalind-franklin/essays/prompt-01.draft.md`
- `schools/rosalind-franklin/essays/prompt-02-challenge-or-setback.md`
- `schools/rosalind-franklin/essays/prompt-02.draft.md`
- `schools/rosalind-franklin/essays/prompt-03-empathy-in-difficult-times.md`
- `schools/rosalind-franklin/essays/prompt-03.draft.md`
- `schools/rosalind-franklin/essays/prompt-04-self-consciousness-and-growth.md`
- `schools/rosalind-franklin/essays/prompt-04.draft.md`
- `schools/rosalind-franklin/essays/prompt-05-two-passions.md`
- `schools/rosalind-franklin/essays/prompt-05.draft.md`
- `schools/rosalind-franklin/essays/prompt-06-optional-dei-statement.md`
- `schools/rosalind-franklin/essays/prompt-06.draft.md`

## School Metadata
- Why-school fact: Chicago Medical School sits within Rosalind Franklin's interprofessional health-sciences campus, which is useful for essays about teamwork and healthcare teams.
- Estimated admit chance: ~6% (possible)
- Estimate basis: Heuristic, not a true admissions probability. Assumes CA resident, GPA 3.75, MCAT 517, and strong research/service/clinical fit. Uses cached 2027 MSAR-style total applicant/interview/matriculant counts (11973 applied, 724 interviewed, 203 matriculated when available), school medians GPA 3.79 / MCAT 510, private or geography-neutral pool, and research alignment, service alignment, leadership alignment.
