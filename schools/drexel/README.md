# Drexel University College of Medicine

- Active list location: Philadelphia, PA
- Active list COA/year: 106067
- Active list GPA median: 3.76
- Active list MCAT median: 512
- Latest prompt cycle used here: 2025-2026 from Admit.org / Med School Insiders
- Latest verified secondary deadline: January 1, 2026
- Official admissions page: https://drexel.edu/medicine/admissions/apply/
- Prompt source: https://med.admit.org/secondary-essays/drexel-university-college-of-medicine

## Required Core Items
- AMCAS primary application
- Official transcripts sent to AMCAS
- Letters of evaluation assigned through AMCAS
- MCAT score on file
- School-specific secondary application when invited

## School-Specific Notes
- Compact prompt set with direct overlap across why-school, current-year, and context essays.
- Secondary prompt count captured in this repo: 3

## Essay Packet
- `schools/drexel/essay-packet.typ`
- `schools/drexel/essays/prompt-01-current-and-recent-activities.md`
- `schools/drexel/essays/prompt-01.draft.md`
- `schools/drexel/essays/prompt-02-why-drexel-and-mission-fit.md`
- `schools/drexel/essays/prompt-02.draft.md`
- `schools/drexel/essays/prompt-03-extenuating-circumstances.md`
- `schools/drexel/essays/prompt-03.draft.md`
