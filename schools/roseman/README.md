# Roseman University College of Medicine

- Active list location: Las Vegas, NV
- Active list COA/year: 113531
- Active list GPA median: 3.79
- Active list MCAT median: 510
- Latest prompt cycle used here: 2025-2026 from SDN school-specific thread
- Latest verified secondary deadline: See secondary invite
- Official admissions page: https://www.roseman.edu/academic-programs/college-of-medicine/doctor-of-medicine-md/
- Prompt source: https://forums.studentdoctor.net/threads/2025-2026-roseman.1508062/

## Required Core Items
- AMCAS primary application
- Official transcripts sent to AMCAS
- Letters of evaluation assigned through AMCAS
- MCAT score on file
- School-specific secondary application when invited

## School-Specific Notes
- Newer Nevada program with surprisingly favorable accepted-stat profile and clear community-accountability language.
- Secondary prompt count captured in this repo: 5

## Essay Packet
- `schools/roseman/essay-packet.typ`
- `schools/roseman/essays/prompt-01-obstacles-on-the-journey-to-medicine.md`
- `schools/roseman/essays/prompt-01.draft.md`
- `schools/roseman/essays/prompt-02-social-determinants-of-health-in-future-practice.md`
- `schools/roseman/essays/prompt-02.draft.md`
- `schools/roseman/essays/prompt-03-cross-cultural-interaction.md`
- `schools/roseman/essays/prompt-03.draft.md`
- `schools/roseman/essays/prompt-04-gap-time-explanation.md`
- `schools/roseman/essays/prompt-04.draft.md`
- `schools/roseman/essays/prompt-05-application-updates.md`
- `schools/roseman/essays/prompt-05.draft.md`

## School Metadata
- Why-school fact: Roseman's Readiness Curriculum is built around social determinants, emerging technology, interprofessional teams, and GENESIS early experiential learning.
- Estimated admit chance: ~24% (comparatively favorable)
- Estimate basis: Heuristic, not a true admissions probability. Assumes CA resident, GPA 3.75, MCAT 517, and strong research/service/clinical fit. Uses cached 2027 MSAR-style total applicant/interview/matriculant counts (792 applied, 268 interviewed, 64 matriculated when available), school medians GPA 3.79 / MCAT 510, private or geography-neutral pool, and research alignment, service alignment, community alignment.
